...
export default function SignInScreen() {
  ...
  async function handleSubmit() {
    if (!login.trim() || !password.trim()) {
      Alert.alert('بيانات ناقصة', 'أدخل اسم المستخدم أو رقم الجوال وكلمة المرور.');
      return;
    }
    try {
      setSubmitting(true);
    ...
    >
      <View style={styles.container}>
        <View style={styles.heroCard}>
          ...
          <Text style={styles.heroSub}>{hasSavedSession ? 'جلسة محفوظة على هذا الجهاز. استخدم الدخول السريع، أو اختر نسيان الجلسة لتسجيل دخول مستخدم آخر.' : 'سجّل الدخول مرة واحدة باسم المستخدم أو رقم الجوال وكلمة المرور، وسيتم حفظ الجلسة آمنًا لاستخدام الدخول السريع في المرات القادمة.'}</Text>
        </View>
        ...
        {shouldShowPasswordForm ? (
          <AppCard title="بيانات الدخول" style={styles.formCard}>
            <LabeledInput label="اسم المستخدم أو رقم الجوال" value={login} onChangeText={setLogin} autoCapitalize="none" placeholder="admin" />
