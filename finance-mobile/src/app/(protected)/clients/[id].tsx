...
import { formatCurrency, formatDate, getClientDisplayStatus, formatDateDDMMYYYY } from '@/utils/format';
import { ownerProfitLabel, ownerTotalProfitLineLabel, ownerMonthlyProfitLabel, partnerProfitLabel, partnerTotalProfitLineLabel, partnerMonthlyProfitLabel, profitShareLabelForUser } from '@/utils/accountLabels';
...
export default function ClientDetailsScreen() {
  ...
  const [expandedSections, setExpandedSections] = useState<Record<ExpandedSection, boolean>>({ profit: false, basic: false });
  ...
  async function handleDelete() {
    ...
    Alert.alert('حذف العميل', `سيتم حذف ${client.name} نهائياً.`, [
      ...
      { text: 'حذف', style: 'destructive', onPress: async () => {
        try { await deleteClient(client.id); router.back(); }
        catch (err) { Alert.alert('تعذر الحذف', err instanceof Error ? err.message : 'حدث خطأ غير متوقع.'); }
      } },
    ]);
  }
  ...
  async function toggleClientStatus() {
    ...
    try { setUpdatingFlags(true); await updateClient(client.id, { status: nextStatus }); await loadData(true); }
    catch (err) { Alert.alert('تعذر تحديث الحالة', err instanceof Error ? err.message : 'حدث خطأ غير متوقع.'); }
    finally { setUpdatingFlags(false); }
  }
  ...
  async function toggleCourtStatus() {
    ...
    try {
      ...
      await updateClient(client.id, { has_court: !client.has_court, court_note: client.has_court ? '' : client.court_note || 'تمت إضافة متابعة قضائية من تطبيق الجوال.' });
      await loadData(true);
    } catch (err) { Alert.alert('تعذر تحديث القضية', err instanceof Error ? err.message : 'حدث خطأ غير متوقع.'); }
    finally { setUpdatingFlags(false); }
  }
  ...
  function openInspectModal(item: PaymentScheduleItem) { setInspectItem(item); }

  async function submitPayment() {
    if (!client || !selectedItem) return;
    try {
      setSavingPayment(true);
      await recordPayment(client.id, { period_key: selectedItem.period_key, paid_amount: paidAmount ? Number(paidAmount) : selectedItem.amount, bank_note: bankNote || null });
      setSelectedItem(null);
      setInspectItem(null);
      await loadData(true);
    } catch (err) { Alert.alert('تعذر التسجيل', err instanceof Error ? err.message : 'حدث خطأ غير متوقع.'); }
  ...
  async function handleRemovePayment(item: PaymentScheduleItem) {
    ...
    Alert.alert('إلغاء الدفعة', `سيتم إلغاء دفعة القسط رقم ${item.month}.`, [
      ...
      { text: 'إلغاء الدفعة', style: 'destructive', onPress: async () => {
        try { await removePayment(client.id, item.period_key, { monthNumber: item.month, paymentId: item.payment_id ?? item.allocation_payment_id ?? item.direct_payment_id }); await loadData(true); }
        catch (err) { Alert.alert('تعذر الإلغاء', err instanceof Error ? err.message : 'حدث خطأ غير متوقع.'); }
      } },
    ]);
  }
    ...
    <Screen
      ...
      rightSlot={
        <View style={styles.headerActions}>
          ...
          {client ? <>
            <IconButton icon="create-outline" accessibilityLabel="تعديل العميل" onPress={() => router.push({ pathname: '/clients/form', params: { id: String(client.id) } })} />
            <IconButton icon="trash-outline" accessibilityLabel="حذف العميل" variant="danger" onPress={handleDelete} />
          </> : null}
        </View>
      }
    >
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void loadData(true); }} />}>
        ...
        {!loading && !error && client ? <>
          ...
          <AccordionSection title="توزيع الأرباح" subtitle={`النوع: ${profitShareLabelForUser(client.profit_share, currentUser)}`} expanded={expandedSections.profit} onToggle={() => toggleSection('profit')}>
            ...
            <KeyValueRow label={ownerTotalProfitLineLabel(currentUser)} value={formatCurrency(client.summary.ahmad_total)} />
            <KeyValueRow label={ownerMonthlyProfitLabel(currentUser)} value={formatCurrency(client.summary.ahmad_monthly)} />
            <KeyValueRow label={partnerTotalProfitLineLabel(currentUser)} value={formatCurrency(client.summary.ali_total)} />
            <KeyValueRow label={partnerMonthlyProfitLabel(currentUser)} value={formatCurrency(client.summary.ali_monthly)} />
          </AccordionSection>

          <AccordionSection title="البيانات الأساسية" subtitle={`${formatDate(client.contract_date)} · ${client.months} شهر`} expanded={expandedSections.basic} onToggle={() => toggleSection('basic')}>
      ...
      <InstallmentDetailsSheet item={inspectItem} visible={Boolean(inspectItem)} onClose={() => setInspectItem(null)} onRecordPress={() => inspectItem && openPaymentModal(inspectItem)} onUndoPress={() => { if (!inspectItem) return; const currentItem = inspectItem; setInspectItem(null); void handleRemovePayment(currentItem); }} />
      <PaymentSheet item={selectedItem} visible={Boolean(selectedItem)} paidAmount={paidAmount} bankNote={bankNote} saving={savingPayment} onClose={() => setSelectedItem(null)} onAmountChange={setPaidAmount} onNoteChange={setBankNote} onSubmit={() => void submitPayment()} />
    </Screen>
  );
}
