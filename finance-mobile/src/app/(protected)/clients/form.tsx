...
import { bondOptions, profitShareOptions as baseProfitShareOptions } from '@/constants/finance';
...
export default function ClientFormScreen() {
  ...
  const accountDisplayName = session?.user?.account_name || session?.user?.account_slug || session?.user?.name || 'الحساب';
  const formatAccountProfitLabel = (value: unknown) => String(value || '').replace(/__ACCOUNT_NAME__|ACCOUNT_NAME|\$\{accountDisplayName\}|أحمد|الحساب/g, accountDisplayName);
  const profitShareOptions = useMemo<any[]>(() => {
    const baseOptions = Array.isArray(baseProfitShareOptions) ? baseProfitShareOptions : [];

    return baseOptions.map((option: any) => ({
      ...option,
      label: formatAccountProfitLabel(option.label),
      subtitle: formatAccountProfitLabel(option.subtitle || ''),
    }));
  }, [accountDisplayName]);
...
/>
          ...
          >
            ...
            <View style={styles.pillsWrap}>
              {profitShareOptions.map((option: any) => {
                ...
                return (
                  ...
                  >
                    <Text style={[styles.compactPillTitle, active && styles.compactPillTitleActive]}>{formatAccountProfitLabel(option.label)}</Text>
            ...
            <View style={styles.segmentedWrap}>
              {[
                ...
                return (
                  ...
                  >
                    <Text style={[styles.segmentText, active && styles.segmentTextActive]}>{formatAccountProfitLabel(item.label)}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
