...
async function shouldSkipBiometricAfterLogout(): Promise<boolean> {
  return false;
}
...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  async function activateSession(nextSession: AuthSession, options: { requireProfile?: boolean } = {}) {
    ...
    try {
      ...
      if (options.requireProfile) {
        ...
        throw new Error('انتهت الجلسة المحفوظة. سجّل الدخول مرة أخرى باسم المستخدم أو رقم الجوال وكلمة المرور.');
      }
  ...
  async function signInWithBiometric() {
    ...
    if (!storedSession?.token) {
      throw new Error('لا توجد جلسة محفوظة. سجّل الدخول أول مرة باسم المستخدم أو رقم الجوال وكلمة المرور.');
    }
    ...
    const result = (await shouldSkipBiometricAfterLogout()) ? { success: false } : await LocalAuthentication.authenticateAsync({
  ...
  async function signOut() {
    await markManualLogoutForBiometrics();
    setSession(null);
    setApiToken(null);
