...
function money(v) { return `${n(v).toLocaleString('ar-SA')} ر.س`; }
function escapeHtml(v) { return String(v ?? '').replace(/[&<>"']/g, ch => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;' }[ch])); }
function statusLabel(s) { return s === 'active' ? 'نشط' : s === 'stuck' ? 'متعثر' : s === 'done' ? 'منتهي' : s === 'cancelled' ? 'ملغي' : String(s || '—'); }
...
function App() {
  ...
  return <View style={s.safe}><StatusBar barStyle="dark-content" /><ScrollView contentContainerStyle={s.wrap}><Header user={user} busy={busy} reload={reload} logout={() => { setToken(''); setUser(null); setClients([]); setAliClients([]); setSelectedClient(null); }} />{selectedClient ? <ClientDetails client={selectedClient} onBack={() => setSelectedClient(null)} /> : <><View style={s.tabs}>{['dashboard','clients','reports'].map(x => <TouchableOpacity key={x} style={[s.tab, tab === x && s.tabOn]} onPress={() => setTab(x)}><Text style={[s.tabText, tab === x && s.tabTextOn]}>{x === 'dashboard' ? 'اللوحة' : x === 'clients' ? 'العملاء' : 'التقارير'}</Text></TouchableOpacity>)}</View>{tab === 'dashboard' ? <Dashboard stats={stats} reports={reports} setTab={setTab} /> : null}{tab === 'clients' ? <ClientsView clients={visibleClients} filter={filter} setFilter={setFilter} query={query} setQuery={setQuery} onSelect={setSelectedClient} /> : null}{tab === 'reports' ? <ReportsView reports={reports} makePdf={makePdf} pdfBusy={pdfBusy} /> : null}</>}</ScrollView></View>;
}
...
function ClientDetails({client,onBack}) { const late = lateInfo(client); const schedule = Array.isArray(client.schedule) ? client.schedule : []; const lateRows = late.rows.slice(0, 6); return <><View style={s.detailsHeader}><TouchableOpacity style={s.backBtn} onPress={onBack}><Text style={s.backText}>رجوع</Text></TouchableOpacity><View style={{flex:1}}><Text style={s.page}>{client.name || 'تفاصيل العميل'}</Text><Text style={s.mutedRight}>عرض فقط بدون تعديل</Text></View></View><View style={s.card}><Text style={s.title}>بيانات العميل</Text><Info label="الاسم" value={client.name || '—'} /><Info label="الجوال" value={client.phone || '—'} /><Info label="الحالة" value={isCourt(client) ? 'قضية' : statusLabel(client.status)} /><Info label="تاريخ العقد" value={fmtDate(client.contract_date || client.created_at)} />{client.court_note ? <Info label="ملاحظة القضية" value={client.court_note} /> : null}</View><View style={s.grid}><Box v={money(monthly(client))} l="القسط الشهري"/><Box v={money(paid(client))} l="المدفوع"/><Box v={money(remaining(client))} l="المتبقي"/></View><View style={s.grid}><Box v={String(late.count)} l="أقساط متأخرة"/><Box v={money(late.amount)} l="المبلغ المتأخر"/><Box v={money(aliProfit(client))} l="ربح علي"/></View><View style={s.card}><Text style={s.title}>معلومات إضافية</Text><Info label="مشاركة الأرباح" value={isShared(client) ? 'يوجد مشاركة' : 'لا يوجد'} /><Info label="نسبة علي" value={client.summary?.ali_pct ? `${client.summary.ali_pct}%` : '—'} /><Info label="عدد الأقساط" value={String(client.months || client.summary?.total_installments || schedule.length || '—')} /></View><View style={s.card}><Text style={s.title}>الأقساط المتأخرة</Text>{lateRows.length ? lateRows.map((r,i) => <View key={`${r.id || i}`} style={s.scheduleRow}><Text style={s.scheduleAmount}>{money(r.remaining_due ?? r.amount ?? monthly(client))}</Text><Text style={s.mutedRight}>{fmtDate(r.due_date)}</Text></View>) : <Text style={s.empty}>لا توجد أقساط متأخرة مسجلة</Text>}</View></>; }
