...
function App() {
  ...
  async function request(path, options = {}) {
    const res = await fetch(`${API}${path}`, { ...options, headers: options.headers || (token ? tokenHeaders(token) : { Accept: 'application/json', 'Content-Type': 'application/json' }) });
  ...
  const reports = useMemo(() => [
    ...
    buildReport('تقرير المتأخرين مع علي', 'التمويلات المتأخرة التي يشارك بها علي في الأرباح فقط.', isAli(user) ? aliClients : clients.filter(c => c.profit_share === 'shared' || Number(c.summary?.ali_total || 0) > 0)),
