...
const headers = (t) => ({ Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${t}` });
...
function installmentAmount(row, c) { return n(row.remaining_due ?? row.amount ?? row.due_amount ?? monthly(c)); }
function scheduleRows(c) { return Array.isArray(c.schedule) ? c.schedule : []; }
function scheduleStats(c) {
  const rows = scheduleRows(c);
  const stats = { paid: 0, late: 0, upcoming: 0, lateAmount: 0 };
  rows.forEach((row) => {
    const st = installmentState(row).key;
    stats[st] += 1;
    if (st === 'late') stats.lateAmount += installmentAmount(row, c);
  });
  if (!rows.length && c.status === 'stuck') { stats.late = 1; stats.lateAmount = monthly(c); }
  return stats;
}
function lateInfo(c) {
  const s = scheduleStats(c);
  return { count: s.late, amount: s.lateAmount, rows: scheduleRows(c).filter((r) => installmentState(r).key === 'late') };
}
...
function App() {
  ...
  async function loadData(nextToken = token, nextUser = user) {
    const allRes = await fetch(`${API}/clients?status=all`, { headers: headers(nextToken) });
    const allJson = await allRes.json().catch(() => ({}));
    if (!allRes.ok) throw new Error(allJson.message || 'تعذر تحميل العملاء');
    let shared = [];
    if (isAli(nextUser)) {
      const pRes = await fetch(`${API}/partner-clients`, { headers: headers(nextToken) });
  ...
  const stats = useMemo(() => {
    const late = clients.map(lateInfo).filter((x) => x.count > 0);
  ...
  const visible = useMemo(() => clients.filter((c) => {
    ...
    if (q && !String(c.name || '').toLowerCase().includes(q) && !String(c.phone || '').includes(q)) return false;
